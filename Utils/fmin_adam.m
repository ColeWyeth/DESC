function [x, fval, exitflag, output] = fmin_adam(fun, x0, m_pos, cum_ind, CoDeg_vec_pos, stepSize, beta1, beta2, epsilon, nEpochSize, options)
...
if (isequal(fun, 'defaults'))
   x = struct('Display', 'final', ...
      ...
      'DerivativeCheck', 'off', ...
      'MaxFunEvals', 1e4, ...
      'MaxIter', 300, ... % I changed this from 1e6
      'TolFun', 1e-6, ...
      'TolX', 1e-5, ...
      'UseParallel', false);
   return;
...
while true
   ...
   % Added by Cole Wyeth: project to simplex
   for l = 1:m_pos
       nsample = CoDeg_vec_pos(l);
       x_new = x((cum_ind(l)+1):cum_ind(l+1));
       w = sort(x_new); 
       Ti = 0; 
       for i = 1:nsample
       ...
       x((cum_ind(l)+1):cum_ind(l+1)) = max(x_new - T, 0);
   end
   ...
   % CW modified stopping condition
