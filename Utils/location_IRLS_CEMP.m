...
wMat = kron(sparse([1:ss_num]',[1:ss_num]', exp(-tau* IRAABVec), ss_num,ss_num,ss_num),speye(d));
...
while (count <= maxit)&&(stagcnt <= staglim)
    ...
    residual_standard = residual_norms./(mean(residual_norms));
   % alpha_gamma=tijMat;
   % alpha_gamma(1,:)=alph.*tijMat(1,:);
   % alpha_gamma(2,:)=alph.*tijMat(2,:);
   % alpha_gamma(3,:)=alph.*tijMat(3,:);
   % tijhat = residual_vec + alpha_gamma;
   % tij_normalized = normc(tijhat);
   % angleMat = tij_normalized.*tijMat;
   % cosangles = sum(angleMat);
   % angles = acos(cosangles);
    Ski = zeros(nsample, m);
    Sjk = zeros(nsample, m);
        for l = IndPos
            i = Ind_i(l); j=Ind_j(l);
            Ski(:,l) = residual_standard(IndMat(i,CoIndMat(:,l)));
            Sjk(:,l) = residual_standard(IndMat(j,CoIndMat(:,l)));
        end
        Smax = Ski+Sjk;
        % compute weight matrix (nsample by m)
        WeightMat = exp(-tau1*Smax);
       % WeightMat = (Smax<1/tau1)+delt;
        ...
        % IR-AAB at current iteration
        IRAABVec = sum(SAABMat,1);
        IRAABVec_standard = IRAABVec/(mean(IRAABVec));
        ES_max = (1-lam)*residual_standard + lam * IRAABVec_standard;
   ...
   %wMat = kron(sparse([1:ss_num]',[1:ss_num]', exp(-tau*residual_norms) ,ss_num,ss_num,ss_num),speye(d));
    ...
    fprintf(' IRLS iteration %d done! NRMSE=%f \n',count, NRMSE);
