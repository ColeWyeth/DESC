...
while 1
   ...
   for l = 1:m % for each edge ij 
       ...
       if norm(grad) > maxGrad
        gradientMags(iter) = norm(grad);
        maxGrad = norm(grad); 
       end 
       wijk(:,l) = wijk(:,l) - (learning_rate/(1 + fix(iter/10))) * grad'; %/(2^fix(iter/50))) * grad';
...
%fprintf('Max gradient norm: %f\n', gradientMags(iter)); 
if iter > 1
    if meanErrors(iter) > meanErrors(iter-1) - min_delta
        strikes = strikes + 1; 
    else
        strikes = 0; 
    end
